...
describeAll('e2e multisig mainnet', () => {
  ...
  for (let n = 0; n < KEYPAIR_SIGNERS * ADDRESSES_NEEDED_PER_SIGNER; n++) {
    const keyPair = deriveKeyPair(deriveHDNode(rootSecretKey!, `m/1234'/${n}`, NETWORK), 0, NETWORK)
  ...
  for (const addressType of addressTypesToTest) {
    describe(addressType, () => {
      ...
      const paymentsConfig: MultisigDogePaymentsConfig = {
        ...commonConfig,
      ...
      const payments = new MultisigDogePayments(paymentsConfig)
      ...
      async function pollUntilFound(signedTx: DogeSignedTransaction) {
      ...
      it(
        ...
        async () => {
          ...
          const sendAmount = '5' // combine small utxos
