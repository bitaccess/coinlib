(function (global, factory) {
  typeof exports === 'object' && typeof module !== 'undefined' ? factory(exports, require('bitcoinjs-lib'), require('@faast/payments-common'), require('request-promise-native'), require('bs58'), require('io-ts'), require('@faast/ts-common'), require('blockbook-client'), require('lodash'), require('crypto'), require('buffer'), require('fs'), require('path'), require('util')) :
  typeof define === 'function' && define.amd ? define(['exports', 'bitcoinjs-lib', '@faast/payments-common', 'request-promise-native', 'bs58', 'io-ts', '@faast/ts-common', 'blockbook-client', 'lodash', 'crypto', 'buffer', 'fs', 'path', 'util'], factory) :
  (global = global || self, factory(global.faastBitcoinPayments = {}, global.bitcoin, global.paymentsCommon, global.request, global.bs58, global.t, global.tsCommon, global.blockbookClient, global.lodash, global.crypto$3, global.buffer, global.fs, global.path, global.util));
}(this, (function (exports, bitcoin, paymentsCommon, request, bs58, t, tsCommon, blockbookClient, lodash, crypto$3, buffer, fs, path, util) { 'use strict';
  ...
  crypto$3 = crypto$3 && crypto$3.hasOwnProperty('default') ? crypto$3['default'] : crypto$3;
  ...
  function retryIfDisconnected(fn, api, logger) {
      return promiseRetry_1((retry, attempt) => {
  ...
  var createHash = crypto$3.createHash;

  var createHmac = crypto$3.createHmac;
  ...
  var base = function (checksumFn) {
    ...
    function encode (payload) {
      ...
      return bs58.encode(Buffer$1.concat([
        payload,
        checksum
      ], payload.length + 4))
    }
  ...
  let binding = bindings_1('secp256k1');
  delete binding.path;
  ...
  var _inBundle = false;
  var _integrity = "sha512-f4x70okzZbIQl/NSRLkI/+tteV/9WqL98zx+SQ69KbXxmVrmjwsNUPn/gYJJ0sHvEak24cZgHIPegRePAtA/xw==";
  ...
  var _where = "/Users/dylan/dev/ba/payments/coin-payments/packages/bitcoin-payments/node_modules/tiny-secp256k1";
  var author = {
  	name: "Fedor Indutny",
  ...
  inherits(JPoint, base$1.BasePoint);
  ...
  inherits(Point$2, base$1.BasePoint);
  ...
  */
  ...
  var tinySecp256k1 = createCommonjsModule(function (module) {

  try {
    module.exports = native_1;
  } catch (err) {
    module.exports = js;
  }
  });
  ...
  function getValueTypeName (value) {
    return native_1$1.Nil(value) ? '' : getTypeName(value.constructor)
  }

  function getValue (value) {
    if (native_1$1.Function(value)) return ''
    if (native_1$1.String(value)) return JSON.stringify(value)
    if (value && native_1$1.Object(value)) return ''
    return value
  }
  ...
  function tfJSON (type) {
    if (native_1$1.Function(type)) return type.toJSON ? type.toJSON() : getTypeName(type)
    if (native_1$1.Array(type)) return 'Array'
    if (type && native_1$1.Object(type)) return 'Object'
  ...
  var _ArrayN = _LengthN.bind(null, native_1$1.Array);
  var _BufferN = _LengthN.bind(null, _Buffer);
  var _HexN = _LengthN.bind(null, Hex);
  var _StringN = _LengthN.bind(null, native_1$1.String);

  function Range (a, b, f) {
    f = f || native_1$1.Number;
  ...
  var TYPES = {
    arrayOf: function arrayOf (type, options) {
      ...
      function _arrayOf (array, strict) {
        if (!native_1$1.Array(array)) return false
        if (native_1$1.Nil(array)) return false
    ...
    maybe: function maybe (type) {
      ...
      function _maybe (value, strict) {
        return native_1$1.Nil(value) || type(value, strict, maybe)
      }
    ...
    map: function map (propertyType, propertyKeyType) {
      ...
      function _map (value, strict) {
        if (!native_1$1.Object(value)) return false
        if (native_1$1.Nil(value)) return false
    ...
    tuple: function tuple () {
      ...
      function _tuple (values, strict) {
        if (native_1$1.Nil(values)) return false
        if (native_1$1.Nil(values.length)) return false
  ...
  function compile (type) {
    if (native_1$1.String(type)) {
      if (type[0] === '?') return TYPES.maybe(type.slice(1))

      return native_1$1[type] || TYPES.quacksLike(type)
    } else if (type && native_1$1.Object(type)) {
      if (native_1$1.Array(type)) {
        if (type.length !== 1) throw new TypeError('Expected compile() parameter of type Array of length 1')
        return TYPES.arrayOf(type[0])
      }

      return TYPES.object(type)
    } else if (native_1$1.Function(type)) {
      return type
    }
  ...
  for (var typeName$2 in native_1$1) {
    typeforce[typeName$2] = native_1$1[typeName$2];
  }
  ...
  class BIP32 {
      ...
      get publicKey() {
          if (this.__Q === undefined)
              this.__Q = tinySecp256k1.pointFromScalar(this.__D, true);
          return this.__Q;
      }
      get privateKey() {
          return this.__D;
      ...
      derive(index) {
          ...
          if (!this.isNeutered()) {
              ...
              const ki = tinySecp256k1.privateAdd(this.privateKey, IL);
              ...
              //    = G*IL + Kpar
              const Ki = tinySecp256k1.pointAddScalar(this.publicKey, IL, true);
      ...
      sign(hash, lowR) {
          ...
          if (lowR === undefined)
              lowR = this.lowR;
          if (lowR === false) {
              return tinySecp256k1.sign(hash, this.privateKey);
          }
          else {
              let sig = tinySecp256k1.sign(hash, this.privateKey);
              ...
              while (sig[0] > 0x7f) {
                  ...
                  sig = tinySecp256k1.signWithEntropy(hash, this.privateKey, extraData);
              }
              return sig;
          }
      }
      verify(hash, signature) {
          return tinySecp256k1.verify(hash, this.publicKey, signature);
      }
  }
  ...
  function fromPrivateKeyLocal(privateKey, chainCode, network, depth, index, parentFingerprint) {
      ...
      network = network || BITCOIN;
      if (!tinySecp256k1.isPrivate(privateKey))
  ...
  function fromPublicKeyLocal(publicKey, chainCode, network, depth, index, parentFingerprint) {
      ...
      if (!tinySecp256k1.isPoint(publicKey))
  ...
  var src_2 = src.fromBase58;
  var src_3 = src.fromPublicKey;
  var src_4 = src.fromPrivateKey;
  ...
  function isValidXprv(xprv, network) {
      try {
          return !src_2(xprv, network).isNeutered();
      }
      catch (e) {
          return false;
      }
  ...
  function isValidXpub(xpub, network) {
      try {
          return src_2(xpub, network).isNeutered();
      }
      catch (e) {
          return false;
      }
  ...
  function deriveHDNode(hdKey, derivationPath, network) {
      const rootNode = src_2(hdKey, network);
