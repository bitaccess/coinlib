...
import { EXTERNAL_ADDRESS, seedAccountsByAddressType, SeedAccountFixture, seedAccountsXPrv } from './fixtures'
...
function runHardcodedPublicKeyTests(
  payments: UHdBitcoinPayments,
  config: HdBitcoinPaymentsConfig,
  accountFixture: SeedAccountFixture,
) {
  const { seed, xpub, addresses, derivationPath } = accountFixture
  it('getFullConfig', () => {
