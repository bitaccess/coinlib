...
import {
  HdDashPayments, HdDashPaymentsConfig, AddressType, SinglesigAddressType,
} from '../src'
...
describe('HdDashPayments', () => {

  describe('static', () => {
    it('should throw on invalid hdKey', () => {
      expect(() => new HdDashPayments({ hdKey: 'invalid' })).toThrow()
    })
  })
  ...
  for (let k in accountsByAddressType) {
    ...
    describe(addressType, () => {
      ...
      describe('hardcoded xpub', () => {
        const config: HdDashPaymentsConfig = {
          hdKey: accountFixture.xpub,
          network: NetworkType.Mainnet,
          addressType,
          logger,
        }
        const payments = new HdDashPayments(config)
...
function runHardcodedPublicKeyTests(
  payments: HdDashPayments,
  config: HdDashPaymentsConfig,
